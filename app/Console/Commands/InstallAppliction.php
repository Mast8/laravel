...
class InstallAppliction extends Command
{
    ...
    {
        $this->line("Puede salir del instalador presionando Ctrl+C cuando asi lo desee.\n");
        $this->createDatabase();
        $this->migrate();
        $this->seed();
        $this->setUpKey();
    ...
    */
    private function createDatabase(){
        ...
        $this->line("Debe escoger un Gestor de base de datos.");
        ...
        $connection = $this->choice('Escoga una base de datos', $available_connections);
        ...
        if($connection == "sqlite"){
            ...
            $this->line('Lo que se encuentra en [] son opciones por defecto,
             solo basta presionar enter para seleccionarla.');
            $host = $this->ask('Host de la base de datos', 'localhost');
            $port = $this->ask('Puerto de la base de datos', $defaultPort);
            $database = $this->ask('Nombre de la base de datos');
            $username = $this->ask('Usuario de la base de datos');
            $password = $this->ask('Contraseña de la base de datos',false);
        }
        ...
        if(!$this->testDbConnection()){
            $this->info("Sus datos ingresados fueron: , $host, $port, $database, $username, $password");
    ...
    */
    private function testDbConnection(){
        $this->line('Revisando conexion a la base de datos.');
        ...
        $this->info('Conexion a la base de datos establecida.');
        return true;
    }
    ...
    */
    private function migrate(){
        $this->line("\nComenzando migracion de la base de datos...");
        $this->call('migrate');
    }
    ...
    */
    private function setUpKey(){
        ...
        $this->info("\nInstalacion del sistema terminada!");
    }
}
